...
//---
   ...
   for(int i=limit;i<rates_total && !IsStopped();i++)
   {
      ...
      if(ot[0]>Signal && ot[1]<NoSignal && ot[2]<NoSignal) Label1Buffer[i-1]=high[i];
      if(ot[0]<NoSignal && ot[1]>Signal && ot[2]<NoSignal) Label1Buffer[i-1]=low[i];
   }
