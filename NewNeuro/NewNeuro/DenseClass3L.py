...
def model_rnn(x_t,y_t,x_e,y_e):
    ...
    with tf.variable_scope("Net"):
        ...
        output = tf.layers.dense(inputs=norm, units=90,activation=tf.nn.sigmoid, name="layer_inp")
        output=tf.contrib.layers.batch_norm(output, center=True, scale=True)
        for i in range(LAYERS):
            output = tf.layers.dense(inputs=output, units=90,activation=tf.nn.sigmoid, name="layer_"+"{}".format(i))
            output=tf.contrib.layers.batch_norm(output, center=True, scale=True)
        
    with tf.variable_scope("predictions"):
        prediction = tf.layers.dense(inputs=output, units=3, activation=None, name="prediction")
        classes=tf.nn.softmax(prediction,name="Classes")
