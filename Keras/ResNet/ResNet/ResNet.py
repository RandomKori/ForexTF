...
for each in flist:
    ...
    tensorboard=keras.callbacks.TensorBoard(log_dir='./logs', histogram_freq=0, batch_size=512, write_graph=True, write_images=False, embeddings_freq=0, embeddings_layer_names=None, embeddings_metadata=None)
    ...
    hist = model.fit(x_train, Y_train, batch_size=batch_size, nb_epoch=nb_epochs,
              verbose=1, validation_data=(x_test, Y_test), callbacks = [reduce_lr,tensorboard])
    saver.save(sess=sess, save_path="./ResNetModel/ResNetModel")
    log = pd.DataFrame(hist.history)
    print ("{0] {1}".format(log.loc[log['loss'].idxmin]['loss'], log.loc[log['loss'].idxmin]['val_acc']))
